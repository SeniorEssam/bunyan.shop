...
var vm = new Vue({
    ...
    methods: {
        ...
        updateCompareProductsQty: function () {
            const cookie = AxiosCart.getCookie('BunyanConstruction.CompareProduct');
            if (cookie !== '') {
        ...
        removeFromCompareList: function (product, index) {
            if (product !== undefined) {
                const compareList = AxiosCart.getCookie('BunyanConstruction.CompareProduct');
                const newCompareList = compareList.replace(product, '');
                AxiosCart.setCookie('BunyanConstruction.CompareProduct', newCompareList);
                vm.compareproducts.Products.splice(index, 1);
            } else {
                AxiosCart.setCookie('BunyanConstruction.CompareProduct', '');
